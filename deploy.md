...
# При входе на страницу потребуется ввести логин (user) и пароль
...
> Если нужно задать конкретный пароль (например: **1996**) и сделать это одной командой без интерактивного ввода, используйте:
>
> ```bash
> # НЕИНТЕРАКТИВНО (создание или перезапись файла с пользователем user и паролем 1996)
> sudo htpasswd -bc /etc/nginx/.htpasswd user 1996
> ```
>
...
> `sudo htpasswd -c /etc/nginx/.htpasswd user`
> # Введите пароль: 1996
> # Повторите пароль: 1996
> ```
>
> После изменения пароля перезагрузите nginx:
>
> ```bash
> sudo nginx -t && sudo systemctl reload nginx
> ```
>
> Проверка доступа (должно вернуть 200 OK):
>
> ```bash
> curl -I -u user:1996 https://ВАШ_ДОМЕН/otchet/views
> ```
>
> Если файл уже существует и вы хотите просто поменять пароль пользователя `user` — не используйте `-c` (чтобы не удалить других пользователей):
>
> ```bash
> sudo htpasswd /etc/nginx/.htpasswd user 1996
> ```
>
> ⚠️ Пароль `1996` очень слабый. Рекомендуется использовать более сложный (например, сгенерировать:
> `openssl rand -base64 18`). Дополнительно можно ограничить доступ по IP или через VPN.
